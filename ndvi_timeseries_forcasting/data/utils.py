...
def combine_resample_era5(
    ...
    for f in filenames:
        ...
        # 1) 处理 ensemble 维度 number（只取第一个成员）
        if "number" in d.dims:
            d = d.isel(number=0, drop=True)

        # 2) 统一时间维度名：valid_time -> time
        if "valid_time" in d.dims:
            d = d.rename({"valid_time": "time"})
        elif "valid_time" in d.coords:
            d = d.rename({"valid_time": "time"})
        elif "valid_time" in d.variables:
            d = d.set_coords("valid_time").rename({"valid_time": "time"})
